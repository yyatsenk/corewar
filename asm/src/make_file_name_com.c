...
static int			make_cor_file(char *name)
{
	...
	name_res = ft_strjoin(no_s, ".cor");
	free(no_s);
	if ((fd = open(name_res, O_WRONLY | O_TRUNC | O_CREAT, 0664)) < 1)
	{
		ft_printf("%&Not enough rights to save file\n", 2);
		free(name_res);
...
static void			put_comment_help(char **arr, t_command *c_copy, int i)
{
	...
	make_binary(&str_binary, program_len, 8);
	while (++i < 8)
	{
