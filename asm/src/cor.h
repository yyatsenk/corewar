...
void					make_binary(char **str_binary, long long num,\
	int num_bytes);
...
t_command				*init_c(char **name, int position,\
	int size, int is_label);
char					*label_cat(char *str, int *end);
char					*comm_cat(char *str, int i);
char					*arg_cat(char *str, int *i);
void					check_maybe_arg(char *str, char *my_str,\
	int i, char *command);
t_op					check_command(char *command, char *str);
void					set_data(t_command *c_copy, char **label_name,\
	int bytes);
...
void					make_evil_warrior(t_command *command, char *arg,\
	char *name, char *comment);
int						lable_is_there(t_command *command);
int						check_line(char *str, t_command *command);
int						check_dir(int *size, t_command *comm);
int						check_ind(char **str, int *size, t_command *command);
int						ft_decimal_code(t_command_code code);
int						one_arg(int a_c, int *size, t_command **c);
int						check_args(t_op command, char *str, t_command **comm,\
	char *str_orig);
int						code_analyse(int fd, char *str, t_command **comm,\
	t_i_j n_c_set);
int						check_name(char *str, char **name, int fd);
int						check_comment(char *str, char **comment, int fd);
int						be_valid_dude(int fd, t_command **command,\
	t_i_j *n_c_set);
