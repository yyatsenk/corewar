...
static int		norm_help_0(char **lable_name, char *value,\
	t_command *comm, t_command *comm_copy)
{
	*lable_name = ft_strsub(value, 1, ft_strlen(comm->str) - 1);
	if (!check_lable_name(*lable_name))
	{
		ft_strdel(lable_name);
		return (0);
	}
	if (check_duplicates(comm->down_un, *lable_name))
	{
		...
		comm->down_un = add_new_u_l(init_u_label(*lable_name, g_num_str,\
			ft_strstr(comm->str_orig, comm->str) - comm->str_orig),\
		comm->down_un);
	}
	return (1);
}
...
static int		norm_help_2(t_command *comm, t_command **comm_copy,\
	long long *num, char **lable_name)
{
	if (!(comm->str))
		return (0);
	*comm_copy = comm;
...
static void		norm_help_3(char **value, int **size,\
	t_command **comm, long long num)
{
...
int				check_dir(int *size, t_command *comm)
{
	...
	if (!(norm_help_2(comm, &comm_copy, &num, &lable_name)))
		return (0);
	...
	else if (!norm_help_1(&num, &value))
	{
		ft_memdel((void **)&(value));
		return (0);
	}
	norm_help_3(&value, &size, &comm, num);
	set_dir(comm, &lable_name);
	ft_memdel((void **)&(comm->str));
	return (1);
}
